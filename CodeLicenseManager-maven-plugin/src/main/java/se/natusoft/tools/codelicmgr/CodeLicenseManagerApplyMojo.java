...
@Authors({
    @Author(
        ...
        changes={
            ...
            @Change(when="2008-12-03", description="Fixed argument structure."),
            @Change(when="2009-01-18", description="Added more mojo arguments for more flexibility."),
            @Change(when="2009-10-31", description="Moved support code to CodeLicenseManager."),
            @Change(when="2009-11-23", description="Now extracts information from pom when not specified in config."),
            @Change(when="2009-11-24", description="Added option to only copy license files for top level projects."),
            @Change(when="2012-05-05", description="Cosmetics only!")
        }
    )
})
